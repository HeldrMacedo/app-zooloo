...
Toda precificação e verificação é feita por triggers no PostgreSQL (banco oficial **`applications`** para dev, e banco **`teste`** obrigatório para testes automatizados) e exposta via
